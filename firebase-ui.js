window.addEventListener("DOMContentLoaded", async () => {
  ...
  const cloudMetaEl = document.getElementById("firebase-cloud-meta");
  ...
  const answeredOf = (payload) => modules?.backup?.getAnsweredCountFromPayload?.(payload) || 0;
  ...
  function updateReminder() {
    ...
    const answered = answeredOf(payload);
...
UID：${masked(currentUser.uid, 4)}`;
      ...
      if (helpText) helpText.textContent = "最簡說明：登入後只會顯示雲端備份資訊；只有當你按『下載雲端備份』時，才會詢問是否載入本機。";
      updateCloudMetaView();
      updateReminder();
      if (!output?.textContent) setOutput("已登入。雲端不會自動載入；若需要再手動下載。
載入前會先保存同步前本機備份，可隨時還原。");
      return;
    }

    if (summaryText) summaryText.textContent = pendingRedirect ? "雲端同步：登入回跳中，請稍候" : "雲端同步：未登入（展開登入選項）";
    if (displayNameEl) displayNameEl.textContent = pendingRedirect ? "登入回跳中..." : "尚未登入";
    if (privateInfoEl) { privateInfoEl.style.display = "none"; privateInfoEl.textContent = ""; }
    ...
    if (helpText) helpText.textContent = pendingRedirect ? "最簡說明：若手機選完 Google 帳戶後仍回到未登入，通常是 GitHub Pages + redirect 被瀏覽器的第三方儲存限制擋住。" : "最簡說明：未登入時仍可正常練題與使用 JSON 匯入匯出；登入只影響雲端同步。";
    updateReminder();
    if (!output?.textContent) setOutput(pendingRedirect ? "正在等待登入回跳結果...若停留很久仍未登入，請改用電腦或改用 popup。" : "請先登入 Google。登入後若要開通雲端同步，請將名稱 / 信箱 / UID 提供給擁有者加入白名單。");
  }
  ...
  try {
    modules = {
      auth: await import("./firebase-auth.js?v=20260330h"),
      smoke: await import("./firebase-sync-smoke.js?v=20260330h"),
      backup: await import("./firebase-backup.js?v=20260330h"),
    };
    ...
    btnLogin?.addEventListener("click", async () => {
      try {
        setOutput("登入中...若手機彈出帳戶選擇後又回到未登入，請先改用電腦登入，或改成 popup-only 部署。" );
...
" + (err?.message || String(err)));
    ...
    btnCloudDownload?.addEventListener("click", async () => {
      try {
        setOutput("下載完整資料中...");
        if (!window.DriverQuizMemory?.applyPayload || !window.DriverQuizMemory?.buildPayload) throw new Error("找不到完整資料匯入／匯出函式。");
        await refreshCloudMeta();
        if (!cloudMeta?.exists) throw new Error("雲端目前沒有備份可下載。");
        let msg = "是否現在載入雲端備份？
";
        if (cloudMeta.updatedAt) msg += `雲端更新時間：${cloudMeta.updatedAt}
`;
        msg += `雲端累計作答：約 ${Number(cloudMeta.answeredCount || 0)} 題
`;
        msg += `本機累計作答：約 ${answeredOf(getCurrentPayloadSafe())} 題

`;
        msg += "按『確定』後，下一步可選擇覆蓋或合併；按『取消』則不載入。";
